...
from winsun.datebase.model import Session, MonthSale, MonthSold, MonthBook, WeekSale, WeekSold, WeekBook
...
class Query:
    ...
    def filter(self, table, date_range, usage=ZHUZHAI, plate=QUANSHI, join=[]):
        """筛选
    ...
    def gxj(self, date_type='week', period=10, usage=ZHUZHAI, plate=QUANSHI, volumn='space', by='trend', book=False):
        ...
        def render_date(date_type, period):
            if date_type == 'week':
                d = Week()
        ...
        def query2df(table, date_range, usage, plate, group_by, outputs):
            res = self.filter(table, date_range, usage, plate)
            res = self.group(res, group_by, outputs)
            return self.to_df(res)

        date_range, index_sql, index_label = render_date(date_type, period)
        ...
        df_sale = query2df(tb_sale, date_range, usage, plate, group_by, outputs)
        ...
        df_sold = query2df(tb_sold, date_range, usage, plate, group_by, outputs)
        ...
        if book:
            ...
            df_book = query2df(tb_book, date_range, usage, plate, group_by, outputs)
